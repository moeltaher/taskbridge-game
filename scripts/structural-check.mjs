...
const state=read('assets/js/core/state.js'),questions=read('assets/js/domain/questions.js'),analysis=read('assets/js/domain/analysis.js'),access=read('assets/js/domain/access.js'),result=read('assets/js/pages/result.js'),components=read('assets/css/components.css'),game=read('assets/css/game.css'),ui=read('assets/js/core/ui.js'),scenarios=read('assets/js/data/scenarios.js');
assert.ok(state.includes('STATE_SCHEMA_VERSION=7'));assert.equal(SCORE_MODEL_VERSION,'6');assert.equal(ECONOMY_MODEL_VERSION,'2');
assert.ok(!questions.includes('scoredQuestionsForState'));assert.ok(!analysis.includes('qScore'));assert.ok(!access.includes('disputeSeverity'));assert.ok(!result.includes('power-targets'));assert.ok(!ui.includes('export function refreshStats'));assert.ok(!scenarios.includes('costModel'));
