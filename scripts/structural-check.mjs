...
const state=read('assets/js/core/state.js'),analysis=read('assets/js/domain/analysis.js'),management=read('assets/js/domain/management.js'),result=read('assets/js/pages/result.js'),ui=read('assets/js/core/ui.js'),power=read('assets/js/pages/power.js');
assert.ok(state.includes('STATE_SCHEMA_VERSION=9'));assert.equal(SCORE_MODEL_VERSION,'8');assert.equal(ECONOMY_MODEL_VERSION,'3');assert.ok(!state.includes('powerTouched'));assert.ok(!power.includes('powerTouched'));assert.ok(analysis.includes('authorityReferenceForState'));assert.ok(management.includes('baselineAcceptanceRate'));assert.ok(result.includes('authorityReferenceForState'));assert.ok(!ui.includes('معدل القبول يحسب قرارات العروض داخل هذه الجولة فقط'));
