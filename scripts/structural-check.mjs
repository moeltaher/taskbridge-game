...
const config=read('assets/js/core/config.js'),routes=read('assets/js/core/routes.js'),state=read('assets/js/core/state.js'),storage=read('assets/js/core/storage.js'),bootstrap=read('assets/js/core/bootstrap.js'),ui=read('assets/js/core/ui.js'),home=read('assets/js/pages/home.js'),power=read('assets/js/pages/power.js'),conclusion=read('assets/js/pages/conclusion.js'),work=read('assets/js/pages/work.js'),management=read('assets/js/pages/management.js'),dispute=read('assets/js/pages/dispute.js'),investigation=read('assets/js/pages/investigation.js'),rights=read('assets/js/pages/rights.js'),result=read('assets/js/pages/result.js'),payment=read('assets/js/pages/payment.js'),access=read('assets/js/pages/access.js'),gameCss=read('assets/css/game.css'),deploy=read('DEPLOY_GITHUB_AR.txt'),workflow=read('.github/workflows/check.yml'),generator=read('scripts/generate-pages.mjs'),syntaxCheck=read('scripts/syntax-check.mjs'),scenarios=read('assets/js/data/scenarios.js'),samples=read('assets/js/data/samples.js'),analysis=read('assets/js/domain/analysis.js'),evidence=read('assets/js/domain/evidence.js'),questions=read('assets/js/domain/questions.js'),domainWork=read('assets/js/domain/work.js'),domainPayment=read('assets/js/domain/payment.js'),domainCheck=read('scripts/domain-check.mjs'),scenarioPage=read('assets/js/pages/scenario.js'),playwright=read('playwright.config.mjs'),e2e=read('tests/e2e/smoke.spec.mjs'),journey=read('tests/e2e/journey.spec.mjs'),pkg=JSON.parse(read('package.json'));
...
assert.ok(!scenarios.includes('priceActor:')&&!scenarios.includes('clientCity:'));
assert.ok(!samples.includes("published:'تهديد'")&&!samples.includes("published:'مسموح'"));
...
assert.ok(!state.includes('export function addEvidence')&&!state.includes('export function addLog')&&!state.includes('export function recalcAcceptance')&&!state.includes('export function stageForPage'));
...
assert.ok(!storage.includes('CURRENT_VERSION=')&&!storage.includes('CURRENT_RESULT_VERSION=')&&!storage.includes('CURRENT_SCORING_VERSION='));
...
assert.ok(home.includes("from '../core/config.js'")&&home.includes("from '../core/html.js'")&&home.includes('resumePage')&&home.includes('outcomeLabel')&&home.includes('stateStorageMode')&&home.includes('<caption'));
assert.ok(!home.includes('const esc=')&&!home.includes('export async function render')&&!home.includes('No Boss v3.0.2'));
assert.ok(rights.includes('consumeCheckpointTo')&&!rights.includes('undoCheckpointTo')&&rights.includes('updateSummary')&&!rights.includes('export async function render'));
...
assert.ok(!domainPayment.includes('workerBeforeCosts:cashPayout'));
assert.ok(domainCheck.includes('data/parties.js')&&domainCheck.includes('data/power-targets.js'));
assert.ok(scenarioPage.includes('freshState')&&scenarioPage.includes('commit({')&&!scenarioPage.includes('addLog')&&!scenarioPage.includes('export async function render'));
...
assert.ok(!workflow.includes('while IFS=')&&!workflow.includes('find assets/js scripts tests'));
