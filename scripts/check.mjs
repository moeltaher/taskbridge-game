...
assert.deepEqual(leaders({worker:10,platform:60,client:30}),['platform']);
assert.deepEqual(secondTier({worker:10,platform:60,client:30}),['client']);
assert.equal(powerAxisCredit({worker:10,platform:60,client:30},ref),1);
assert.equal(powerAxisCredit({worker:30,platform:60,client:10},ref),.7);
assert.equal(powerAxisCredit({worker:0,platform:50,client:50},{primary:['platform','client'],secondary:[]}),1);
const localStore=new Map(),sessionStore=new Map();
globalThis.localStorage={getItem:k=>localStore.has(k)?localStore.get(k):null,setItem:(k,v)=>localStore.set(k,String(v)),removeItem:k=>localStore.delete(k)};
...
assert.equal('reviewTaskScore' in getState(),false);assert.equal('unknownLegacy' in getState(),false);
const storage=await import('../assets/js/core/storage.js');
localStore.set('no_boss_results','{}');assert.deepEqual(storage.savedResults(),[]);
