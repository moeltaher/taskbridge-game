...
const profiles={
 wellbeing:{occurred:true,icon:'🧠',kind:'وقت تعافٍ صحي ونفسي مرتبط بالعمل',title:'ظهرت آثار التعرض للمحتوى الذي راجعته',cause:'تضمنت العينات التي راجعتها في هذه الجولة إساءة أو تهديدًا. بعد إنهاء العمل احتجت إلى 3 دقائق بعيدًا عن المنصة قبل متابعة الوردية.',minutes:3,stress:18,consequence:'سجلت المحاكاة 3 دقائق كوقت تعافٍ مرتبط بالعمل بلا مقابل مستقل.'},
...
export function riskTransition(scenario,state){
 ...
 const completedTasks=event.technicalIssue&&event.affectedTaskId?(state.completedTasks||[]).map(task=>task.id===event.affectedTaskId?{...task,technicalIssue:true}:task):(state.completedTasks||[]);
