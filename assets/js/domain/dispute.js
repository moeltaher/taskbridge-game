...
export function availableAppealGrounds(scenario,state,taskOverride=null){
 ...
 if(scenario.type==='data'){const ids=['guideline'];if(task?.technicalIssue===true)ids.push('technical');return Object.fromEntries(ids.map(id=>[id,appealGrounds[id]]))}
 const profile=reviewProfile(scenario,task),ids=Object.keys(profile.groundCounts||{}).filter(id=>profile.groundCounts[id]>0);
 return Object.fromEntries(ids.map(id=>[id,appealGrounds[id]]).filter(([,label])=>label));
}
function groundMatches(scenario,ground,{reviewableErrors=0,hardErrors=0,technicalIssues=0,groundCounts={}}={}){
 if(!ground)return false;
 if(scenario.type==='data'){if(ground==='technical')return technicalIssues>0;if(ground==='guideline')return reviewableErrors>0&&hardErrors<=1;return false}
 return Number(groundCounts[ground]||0)>0&&hardErrors<=1;
}
function dataReviewProfile(task){let reviewableErrors=0,hardErrors=0;task.answers.forEach((answer,i)=>{const credit=dataAnswerCredit(task.sampleIndexes[i],answer);if(credit>=.55&&credit<.85)reviewableErrors++;else if(credit<.55)hardErrors++});return {reviewableErrors,hardErrors,technicalIssues:task.technicalIssue===true?1:0,groundCounts:{guideline:reviewableErrors}}}
export function reviewProfile(scenario,task){
 if(!task)return {reviewableErrors:0,hardErrors:0,technicalIssues:0,groundCounts:{}};
 ...
 return {reviewableErrors,hardErrors,technicalIssues:0,groundCounts};
}
export function reviewAppeal(scenario,state){
 ...
 if(!availableAppealGrounds(scenario,state,task)[ground])return {accepted:false,finalSeverity:initial,reason:'سبب الاعتراض المختار غير مدعوم بأي اختلاف قابل للمراجعة في هذه المهمة.',reviewableErrors:0,hardErrors:0,ground};
 ...
 const matched=Number(profile.groundCounts?.[ground]||0);
 return {accepted,finalSeverity:reviewedSeverity(initial,accepted),...profile,ground,reason:accepted?`وجدت المراجعة ${matched} اختلافًا قابلًا للدفاع يدعم سبب الاعتراض المحدد (${appealGrounds[ground]}). لذلك خُفضت شدة القرار درجة واحدة.`:`لا يوجد اختلاف قابل للمراجعة يدعم السبب المحدد (${appealGrounds[ground]}) بما يكفي لتغيير النتيجة.`};
}
