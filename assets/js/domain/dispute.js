...
export function reviewAppeal(scenario,state){
 ...
 const profile=reviewProfile(scenario,task),counterfactualScore=counterfactualAppealScore(scenario,task,ground),finalSeverity=reviewSeverityFromScore(counterfactualScore),accepted=finalSeverity<initial,matched=scenario.type==='data'?(ground==='technical'?profile.technicalIssues:profile.reviewableErrors):Number(profile.groundCounts?.[ground]||0);
