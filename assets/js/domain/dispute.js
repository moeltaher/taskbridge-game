...
export function applyDisputeOutcome(state){const qualityBefore=state.qualityBeforeDispute??state.quality,accessBefore=state.accessBeforeDispute??state.access,consequences=disputeConsequences(state,state.disputeSeverity,state.appealed),qualityAfter=clamp(qualityBefore-consequences.penalty,0,100),accessAfter=clamp(accessBefore-consequences.penalty,0,100);return {consequences,qualityBefore,accessBefore,qualityAfter,accessAfter,changes:{hold:consequences.hold,quality:qualityAfter,access:accessAfter,qualityBeforeDispute:qualityBefore,accessBeforeDispute:accessBefore,disputeFinalized:true,disputeResult:{severity:state.disputeSeverity,appealed:state.appealed,hold:consequences.hold,penalty:consequences.penalty,qualityBefore,qualityAfter,accessBefore,accessAfter,appealCost:state.appealCost},stage:6,status:'قيد التسوية',payment:null}}}

export function appealCostChanges(state){return {appealCostApplied:true,appealCost:{minutes:APPEAL_TIME_MINUTES,stress:APPEAL_STRESS},time:state.time+APPEAL_TIME_MINUTES,extraWorkTime:Number(state.extraWorkTime||0)+APPEAL_TIME_MINUTES,stress:clamp(state.stress+APPEAL_STRESS,0,100)}}
