...
export function assessAccessDecision(sc,state){
 const finalSeverity=Number(state.finalReviewSeverity??0);
 ...
 const rejectionFactor={title:'سجل رفض العروض',value:`${state.rejections} مرة`,points:rejectionPoints(state.rejections),why:state.noWorkEnding?'رفض العروض يمكن أن يؤدي إلى تحذير في هذا المسار، لكنه لا يخفض مؤشرًا عدديًا خفيًا ولا يستطيع تقييد «مشروع حالي» عندما لم يقبل العامل مشروعًا أصلًا.':'رفض واحد يضيف نقطة ورفضان يضيفان نقطتين. لا يستخدم القرار معدل القبول أو مؤشر الوصول المرحلي مرة أخرى.'};
 const factors=[reviewFactor,rejectionFactor],points=factors.reduce((sum,item)=>sum+item.points,0);
 const projectAt=sc.accessPolicy?.projectAt??3,suspendAt=sc.accessPolicy?.suspendAt??4;
 let outcome;
