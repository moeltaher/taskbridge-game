...
export function computeManagedAccess(scenario,state){let qualityWeight=.60,acceptanceWeight=.30,base=10;if(scenario.type==='translation'){qualityWeight=.42;acceptanceWeight=.18;base=28}else if(scenario.type==='moderation'){qualityWeight=.62;acceptanceWeight=.28}else if(scenario.type==='ai'){qualityWeight=.55;acceptanceWeight=.25}return clamp(Math.round(base+state.quality*qualityWeight+state.acceptance*acceptanceWeight),35,95)}
...
export function secondOfferDecision(state,accepted){const offer=state.secondOffer,before={acceptance:state.acceptance,access:state.access,stress:state.stress};const offerDecisions=state.offerDecisions+1,acceptedOffers=state.acceptedOffers+(accepted?1:0),rejections=state.rejections+(accepted?0:1),acceptance=acceptanceRate(acceptedOffers,offerDecisions);return {offerDecisions,acceptedOffers,rejections,acceptance,result:{accepted,completed:false,title:offer.title,pay:offer.pay,duration:offer.duration,beforeAcceptance:before.acceptance,afterAcceptance:acceptance,beforeAccess:before.access,afterAccess:before.access,beforeStress:before.stress,afterStress:before.stress}}}
export function completeSecondTask(state){const offer=state.secondOffer,decision=state.offerDecisionResult,stressDelta=secondTaskStressDelta(offer),afterStress=clamp(state.stress+stressDelta,0,100);return {stressDelta,changes:{grossWorker:state.grossWorker+offer.pay,clientPaid:state.clientPaid+offer.clientValue,time:state.time+offer.duration,paidTime:state.paidTime+offer.duration,stress:afterStress,offerDecisionResult:{...decision,completed:true,afterStress},managementStep:'offerResult',status:'اكتملت المهمة الثانية'}}}
