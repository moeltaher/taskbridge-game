...
function answerCredit(sample,answer){if(answer===sample.preferred)return 1;return sample.acceptable?.includes(answer)?.75:0}
export function scoreWork(scenario,answers){const count=Math.max(1,answers.length);if(scenario.type==='data')return clamp(Math.round(answers.reduce((sum,answer,index)=>sum+intersectionOverUnion(answer,DATA_TARGETS[index]),0)/count*100),0,100);const pool=samples[scenario.type];return Math.round(answers.reduce((sum,answer,index)=>sum+answerCredit(pool[index],answer),0)/count*100)}
export function firstTaskOutcome(scenario,state){const score=scoreWork(scenario,state.workAnswers),quality=clamp(Math.round((state.quality*2+score)/3),0,100),job=state.selectedJob,stressDelta=scenario.type==='moderation'?16:10,stressBefore=state.stress,stressAfter=clamp(stressBefore+stressDelta,0,100);return {score,job,stressDelta:stressAfter-stressBefore,changes:{workScore:score,quality,qualityAfterFirstTask:quality,time:state.time+job.duration,paidTime:state.paidTime+job.duration,grossWorker:state.grossWorker+job.pay,clientPaid:state.clientPaid+job.clientValue,stress:stressAfter,firstTaskStress:{before:stressBefore,after:stressAfter,delta:stressAfter-stressBefore},status:'اكتملت المهمة الأولى',workStep:'result'}}}
