...
const DATA_WIDTH={narrow:.24,medium:.34,wide:.44};
...
export const dataWidthOptions=[{id:'narrow',label:'صندوق ضيق'},{id:'medium',label:'صندوق متوسط'},{id:'wide',label:'صندوق عريض'}];
...
export function semanticDataAnswer(sceneIndex,regionId,widthId='medium'){const target=DATA_TARGETS[sceneIndex],center=DATA_REGION_X[regionId],w=DATA_WIDTH[widthId];if(!target||!Number.isFinite(center)||!Number.isFinite(w))return null;const x=clamp(center,0,1-w);return {x,y:target.y,w,h:target.h,regionId,widthId,source:'semantic'}}
...
export function dataReviewMetrics(answers,indexes){return answers.map((answer,i)=>intersectionOverUnion(answer,DATA_TARGETS[indexes[i]]))}
...
export function taskRecord({id,job,sampleIndexes,answers,score,qualityBefore,qualityAfter,stressDelta,reviewMetrics=null}){return {id,jobId:job.id||id,title:job.title,sampleIndexes:[...sampleIndexes],answers:structuredClone(answers),score,pay:Number(job.pay),clientValue:Number(job.clientValue),duration:Number(job.duration),stressDelta:Number(stressDelta||0),qualityBefore:Number(qualityBefore),qualityAfter:Number(qualityAfter),reviewMetrics:reviewMetrics?reviewMetrics.map(Number):null}}
export function firstTaskOutcome(scenario,state){const indexes=state.currentTaskSampleIndexes?.length?state.currentTaskSampleIndexes:Array.from({length:state.workAnswers.length},(_,i)=>i),score=scoreWork(scenario,state.workAnswers,{sampleIndexes:indexes}),qualityBefore=state.quality,quality=qualityAfterTask(qualityBefore,score),job=state.selectedJob,stressDelta=Number(job.stress??scenario.jobStress?.[job.id]??(scenario.type==='moderation'?16:10)),stressBefore=state.stress,stressAfter=clamp(stressBefore+stressDelta,0,100),record=taskRecord({id:'task-1',job,sampleIndexes:indexes,answers:state.workAnswers,score,qualityBefore,qualityAfter:quality,stressDelta:stressAfter-stressBefore,reviewMetrics:scenario.type==='data'?dataReviewMetrics(state.workAnswers,indexes):null});return {score,job,record,stressDelta:stressAfter-stressBefore,changes:{workScore:score,quality,qualityAfterFirstTask:quality,time:state.time+job.duration,paidTime:state.paidTime+job.duration,grossWorker:state.grossWorker+job.pay,clientPaid:state.clientPaid+job.clientValue,disputedTaskPay:job.pay,stress:stressAfter,firstTaskStress:{before:stressBefore,after:stressAfter,delta:stressAfter-stressBefore},completedTasks:[...(state.completedTasks||[]),record],sampleCursor:Number(state.sampleCursor||0)+indexes.length,status:'اكتملت المهمة الأولى',workStep:'result'}}}
