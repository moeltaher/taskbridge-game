...
export function analysisAxes(state){
 ...
 if(state?.noWorkEnding)return axes.filter(axis=>['price','allocation','risk','termination'].includes(axis.id));
 return axes;
}
export function evidenceDimensions(state){
 if(state?.contractDeclineEnding)return CONTRACT_DECLINE_DIMENSIONS;
 return state?.noWorkEnding?NO_WORK_EVIDENCE_DIMENSIONS:WORK_EVIDENCE_DIMENSIONS;
}
export function powerMapComplete(state){return analysisAxes(state).every(axis=>(state.powerTouched||[]).includes(axis.id))}
function evidenceCredit(evidence,selected){
 if(evidence.scoreable===false)return null;
 if(selected===evidence.preferredKind)return 1;
 if((evidence.validKinds||[]).includes(selected))return .5;
 return 0;
}
export function scoreAnalysis(scenario,state){
 ...
 for(const id of state.evidence){
  const evidence=evidenceFor(id,scenario,state),credit=evidenceCredit(evidence,state.evidenceSort[id]);
  if(credit!==null&&dimensions.has(evidence.dimension))dimensions.get(evidence.dimension).push(credit);
 }
 ...
 const sortScore=evidenceTotal?Math.round(evidenceCorrect/evidenceTotal*40):40;
 const activeAxes=analysisAxes(state);
 let powerRaw=0;
 for(const axis of activeAxes)powerRaw+=powerAxisCredit(state.power[axis.id],authorityReference(scenario.type,axis.id));
