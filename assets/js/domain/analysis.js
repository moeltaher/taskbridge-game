...
export function analysisAxes(state){
 ...
 if(state?.noWorkEnding)return axes.filter(axis=>['price','allocation','risk','termination'].includes(axis.id)).map(axis=>axis.id==='price'?{...axis,title:'قواعد تحديد السعر المعروضة في السوق'}:axis.id==='allocation'?{...axis,title:'قواعد توزيع فرص العمل المعروضة في السوق'}:axis);
 return axes;
}
...
export function evidenceCredit(evidence,selected){if(evidence.scoreable===false)return null;if(selected===evidence.preferredKind)return 1;if((evidence.validKinds||[]).includes(selected))return .5;return 0}
