...
import {APPEAL_TIME_MINUTES,appealGrounds,availableAppealGrounds,reviewSeverityFromScore,selectReviewTask,reviewAppeal,disputeConsequences,applyDisputeOutcome,appealCostChanges,publishedTranslationText} from '../domain/dispute.js';
...
function referenceFor(sc,task){
 ...
 if(sc.type==='data')return {worker:`${taskLabel(task)}: أرسلت ${task.sampleIndexes.length} إطارات تحديد، والنتيجة ${task.score}%.`,client:`المراجعة تقارن إطارات هذه المهمة بالحدود المرجعية للمركبات.${task.technicalIssue===true?' هذه المهمة نفسها تحمل واقعة تقنية مسجلة مرتبطة بعرض/مزامنة الإجابة.':''}`};
 let i=task.answers.findIndex((answer,j)=>!samples[sc.type][task.sampleIndexes[j]]?.acceptable?.includes(answer));if(i<0)i=0;
 const sample=samples[sc.type][task.sampleIndexes[i]],answer=task.answers[i];
 if(sc.type==='moderation')return {worker:`${sample.text}<br><b>قرارك:</b> ${answer??'—'}`,client:`<b>الاختيار المرجعي:</b> ${sample.preferred}<br><span class="small">قابل للمراجعة الثانية: ${(sample.reviewable||[]).join(' أو ')||'لا يوجد'}</span>`};
 if(sc.type==='ai')return {worker:`${sample.q}<br><b>اختيارك:</b> ${answer??'—'}`,client:`<b>الإجابة المرجعية:</b> ${sample.preferred}`};
 return {worker:`${sample.src}<br><b>اختيارك:</b> ${answer??'—'}`,client:`<b>الصياغة المفضلة:</b> ${publishedTranslationText(sample)}<br><span class="small">القابل للمراجعة الثانية: ${(sample.reviewable||[]).join(' أو ')||'لا يوجد'}</span>`};
}
function registerAppeal(root,sc){
 ...
 commit({changes:{...cost,appealed:true,appealReview:review,finalReviewSeverity:review.finalSeverity},evidence:['appeal'],log:{title:'طلبت مراجعة إضافية',text:`سبب الاعتراض: ${appealGrounds[state.appealGround]}. ${review.reason}`}});
 render(root);
}
function finalize(){
...
export function render(root){
 ...
 const current=getState(),ref=referenceFor(sc,task),noAppeal=disputeConsequences(current,severity),grounds=availableAppealGrounds(sc,current,task);
