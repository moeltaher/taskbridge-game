...
function sortButtons(e,id,sel){
 if(e.dimension==='burden')return `<button class="sort ${sel==='burden'?'sel':''}" aria-pressed="${sel==='burden'}" data-id="${id}" data-v="burden">دليل على تحمّل العامل للعبء</button><button class="sort ${sel==='ind'?'sel':''}" aria-pressed="${sel==='ind'}" data-id="${id}" data-v="ind">دليل على استقلال العامل</button><button class="sort ${sel==='dep'?'sel':''}" aria-pressed="${sel==='dep'}" data-id="${id}" data-v="dep">دليل مختلط / يحتاج سياقًا</button>`;
 return `<button class="sort ${sel==='ctrl'?'sel':''}" aria-pressed="${sel==='ctrl'}" data-id="${id}" data-v="ctrl">دليل على سلطة أحد الأطراف</button><button class="sort ${sel==='ind'?'sel':''}" aria-pressed="${sel==='ind'}" data-id="${id}" data-v="ind">دليل على استقلال العامل</button><button class="sort ${sel==='dep'?'sel':''}" aria-pressed="${sel==='dep'}" data-id="${id}" data-v="dep">دليل مختلط / يحتاج سياقًا</button>`;
}
function evidenceView(sc,s){
 const ids=scoreableIds(s,sc),done=ids.filter(id=>s.evidenceSort[id]).length;
 return `<div class="task-now"><span>🎯 مهمتك الآن</span><b>صنّف الأدلة بحسب ما تثبته فعلًا: سلطة، استقلال، عبء يتحمله العامل، أو دلالة مختلطة.</b></div><div class="notice info"><b>التقدم في الأدلة المحسوبة:</b> ${done}/${ids.length}. أدلة العبء لها اختيار صريح مستقل؛ لم تعد واقعة تحمل التكلفة تُجبر على خانة «مختلط». أفضل قراءة تحصل على الدرجة الكاملة، والقراءة البديلة القابلة للدفاع تحصل على درجة جزئية. عناصر التسوية المالية الموسومة «غير محسوب» لا تدخل هذه الدرجة.</div><div class="evidence-grid">${s.evidence.map(id=>{const e=evidenceFor(id,sc,s),sel=s.evidenceSort[id];if(e.scoreable===false)return `<div class="doc"><span class="pill">${dimensionLabel[e.dimension]||e.dimension} · غير محسوب</span><b>${e.title}</b><p class="small">${e.text}</p><div class="notice info small">هذه معلومة تفسيرية عن حركة الأموال، وليست سؤالًا عن سلطة العمل أو استقلال العامل.</div></div>`;return `<div class="doc"><span class="pill">${dimensionLabel[e.dimension]||e.dimension}</span><b>${e.title}</b><p class="small">${e.text}</p><div class="ev-actions" role="group" aria-label="تصنيف دليل ${e.title}">${sortButtons(e,id,sel)}</div></div>`}).join('')}</div><div class="actions"><button class="btn" data-next="questions">انتقل إلى الأسئلة التشخيصية</button></div>`;
}
function questionView(s){
 const questions=questionsForState(s);
 return `<div class="task-now"><span>🎯 مهمتك الآن</span><b>اختبر الفروق المفاهيمية التي ستحتاجها قبل بناء الخريطة.</b></div><div class="notice info"><b>هذه الأسئلة تشخيصية وغير محسوبة في الدرجة.</b> وهي لا تعيد سؤال الخريطة محورًا بمحور؛ بل تختبر الفرق بين العبء والسلطة، وبين سلطة المشروع والحساب، وبين التسعير والتمويل، وبين الترتيب المرحلي والقرار النهائي.</div><p class="small muted">${s.contractDeclineEnding?'هذا المسار يختبر فقط معنى بوابة الدخول التعاقدية.':s.noWorkEnding?'لا تظهر أسئلة عن مراقبة أو مراجعة أو تسوية لم تحدث، ولا تنسب للعميل قرار استبعاد لم يقع.':'التسوية المالية تُسأل كمفهوم مستقل حتى لا تختلط بدور إدارة العمل.'}</p><div class="qgrid">${questions.map(({id,title,options})=>`<div class="qcard"><label for="q-${id}"><b>${title}</b></label><span class="pill">تشخيصي · غير محسوب</span><select id="q-${id}" data-q="${id}"><option value="">اختر بناء على الوقائع</option>${options.map(option=>`<option ${s.answers[id]===option?'selected':''}>${option}</option>`).join('')}</select></div>`).join('')}</div><div class="actions"><button class="btn" id="toPower">ابن خريطة السلطة والعبء</button></div>`;
}
