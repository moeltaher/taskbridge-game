...
function monitor(root,sc,s){
 ...
 root.innerHTML=`<div class="panel"><div class="task-now decision"><span>⚖️ قرار قبل الإغلاق</span><b>${taskText}</b></div><div class="notice info"><b>معلومة تكشفها المنصة الآن:</b> أثناء العمل سجّلت No Boss ${monitoringCopy(sc)}. لم تكن تفاصيل هذا الرصد معروضة لك قبل التنفيذ؛ هذا التأخر مقصود في المحاكاة لتوضيح أثر نقص الشفافية.</div><div class="grid-2"><div class="card"><small>جودة الحساب الحالية</small><h3>${s.quality}%</h3></div><div class="card"><small>عبء الوردية</small><h3>${s.stress}/100 · ${wellbeingLabel(s.stress)}</h3></div></div><div class="notice"><b>أثر الاستراحة معلن:</b> تضيف ${BREAK_MINUTES} دقائق وتخفض العبء حتى ${BREAK_STRESS_REDUCTION} نقاط. زر الرجوع في المرحلة التالية سيظهر بوضوح كتراجع عن هذا القرار إذا أردت إعادة اختياره.</div><div class="actions"><button class="btn secondary" id="break">استراحة ${BREAK_MINUTES} د · عبء -${BREAK_STRESS_REDUCTION}</button><button class="btn" id="continue">الانتقال إلى المراجعة دون استراحة</button></div></div>`;
