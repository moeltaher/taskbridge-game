...
function ensureRisk(scenario){
 ...
 if(transition.changes)commit({changes:transition.changes,evidence:transition.event.occurred?['risk']:[],log:{title:transition.event.occurred?'حدث موقف إضافي مرتبط بالعمل':'لم يقع حدث إضافي',text:transition.event.occurred?`${transition.event.title}: أضيفت ${transition.event.minutes} دقيقة إلى الوقت الإضافي المرتبط بالعمل.`:'انتهت هذه الجولة دون حادث إضافي بعد التسليم؛ الخطر البنيوي لا يعني وقوع الضرر حتمًا.'}});
...
function affectedTaskLabel(state,risk){
 if(!risk.technicalIssue||!risk.affectedTaskId)return'';
 const task=(state.completedTasks||[]).find(t=>t.id===risk.affectedTaskId);
 return `<div class="notice good"><b>ربط الواقعة بالمهمة:</b> ${task?.id==='task-2'?'المهمة الثانية':'المهمة الأولى'} هي التي تحمل علامة العطل التقني. لن يصلح هذا العطل سببًا لاعتراض على مهمة أخرى.</div>`;
}
