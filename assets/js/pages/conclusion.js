...
function updateRoles(root){
 const support=[],counter=[],dual=[];
 root.querySelectorAll('[data-evidence-role]').forEach(select=>{if(select.value==='support')support.push(select.dataset.evidenceRole);if(select.value==='counter')counter.push(select.dataset.evidenceRole);if(select.value==='dual')dual.push(select.dataset.evidenceRole)});
 patch({conclusionEvidence:support,conclusionCounterEvidence:counter,conclusionDualEvidence:dual});
}
function conclusionBrief(state){
 if(state.contractDeclineEnding)return {prompt:'اكتب استنتاجًا عن بوابة الدخول التعاقدية: من يملك قرار الدخول إلى السوق، وما الذي يقوله رفض الشروط عن القدرة على التفاوض؟ اذكر أي واقعة تحد هذا الاستنتاج إن وجدت.',minimum:1};
 if(state.noWorkEnding)return {prompt:'اكتب استنتاجًا عن دخول السوق دون الحصول على عمل: من يحدد العروض والوصول، ومن تحمل وقت البحث وتكاليف المشاركة رغم غياب الدخل؟',minimum:1};
 return {prompt:'اكتب استنتاجًا يحدد أين تتركز السلطة، من يتحمل العبء، وما الوقائع التي تجعل الصورة أكثر تعقيدًا إن وجدت.',minimum:Math.min(2,state.evidence.length)};
}
export function render(root){
 ...
 document.getElementById('finish').onclick=()=>{
  const current=getState(),text=current.analysisText.trim(),supportCount=(current.conclusionEvidence||[]).length+(current.conclusionDualEvidence||[]).length;
  if(!powerMapComplete(current)){alert(`ارجع إلى خريطة السلطة والعبء ووزع المحاور ${axesCount} الظاهرة في هذه الجولة قبل إظهار النتيجة.`);return}
  if(text.length<40){alert('اكتب استنتاجًا تحليليًا من 40 حرفًا على الأقل يربط الاستنتاج بوقائع الجولة.');return}
  if(supportCount<brief.minimum){alert(`اختر ${brief.minimum} ${brief.minimum===1?'عنصرًا داعمًا':'عنصرين داعمين'} على الأقل.`);return}
  const all=[...(current.conclusionEvidence||[]),...(current.conclusionCounterEvidence||[]),...(current.conclusionDualEvidence||[])];
  if(new Set(all).size!==all.length){alert('اختر دورًا واحدًا فقط لكل عنصر؛ استخدم «يدعمه ويعقده معًا» للدلالة المزدوجة.');return}
  const missing=current.evidence.filter(id=>{const e=evidenceFor(id,scenario,current);return e.scoreable!==false&&!current.evidenceSort[id]});
  if(missing.length){alert('ارجع إلى تصنيف الأدلة وصنف جميع الأدلة المحسوبة قبل إظهار النتيجة.');return}
  commit({changes:{resultData:scoreAnalysis(scenario,current),stage:11},checkpointLabel:'العودة إلى الاستنتاج التحليلي'});
