...
export function normalizeState(value){
 ...
 if(!value||typeof value!=='object'||Array.isArray(value)||Number(value.schemaVersion)!==STATE_SCHEMA_VERSION)return base;
...
const saved=loadState();let state=saved?normalizeState(saved):freshState();revisionCounter=Number(state.storageRevision)||0;
const initialMode=stateStorageMode();lastPersistence=initialMode==='session'?{ok:true,status:'session',persistent:false,session:true}:{ok:true,status:'persistent',persistent:true,session:false};
...
export function setState(next){syncExternalState();state=normalizeState(structuredClone(next));revisionCounter=Math.max(revisionCounter,Number(state.storageRevision)||0);persist();return state}
function checkpoint({persistNow=true,label='العودة إلى الحالة السابقة'}={}){const snap=structuredClone(state);snap.checkpoints=[];state.checkpoints.push({page:state.currentPage,label,snapshot:snap});if(state.checkpoints.length>30)state.checkpoints.shift();if(persistNow)persist();return state}
export function commit({changes={},evidence=[],log=null,checkpointLabel=null}={}){syncExternalState();const nextStage=Object.prototype.hasOwnProperty.call(changes,'stage')?Number(changes.stage):Number(state.stage),stageChanges=Number.isFinite(nextStage)&&nextStage!==Number(state.stage);if(stageChanges||checkpointLabel)checkpoint({persistNow:false,label:checkpointLabel||'العودة إلى المرحلة السابقة'});let dirty=applyChanges(changes);for(const id of evidence){if(!state.evidence.includes(id)){state.evidence.push(id);dirty=true}}if(log){appendLog(log);dirty=true}if(dirty)persist();return state}
